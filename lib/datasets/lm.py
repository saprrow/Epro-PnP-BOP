...
class LM(data.Dataset):
    ...
    def vis_anno(self, idx):
        ...
        rgb = self.load_rgb(idx)[:,: ,::-1]
        res = self.cfg.dataiter.out_res
        K = ref.K
        out = out.transpose(1,2,0)
        coor = out[:,:,:3]
        coor = self.unnorm_coor(coor, obj_id)
        r, t = coor2pose1(coor, c, s, res, out[:,:,-1], K)
        R_matrix = cv2.Rodrigues(r, jacobian=0)[0]
        pose_cor = pose.copy()
        pose_cor[:,:3] = R_matrix
        img_pose_coor_vis = self.vis_pose_vertex(obj_id, pose_cor, rgb.copy())
        img_pose_vis = self.vis_pose_vertex(obj_id, pose, rgb.copy())
        fig, ax = plt.subplots(2,1) 
        ax[0].imshow(img_pose_coor_vis)
        ax[0].set_title("coor pose")
        ax[1].imshow(img_pose_vis)
        ax[1].set_title("ori pose")
        plt.show()
    ...
    def vis_pose_vertex(self, obj_id, pose, img):
        ...
        obj_x = self.infos[obj_id]["min_x"] * 1000 * -2
        obj_y = self.infos[obj_id]["min_y"] * 1000 * -2
        obj_z = self.infos[obj_id]["min_z"] * 1000 * -2
        obj_dim = [obj_x, obj_y, obj_z]
        return draw_pose_vertex(img, obj_dim, r_vector, t_vector, ref.K) 
